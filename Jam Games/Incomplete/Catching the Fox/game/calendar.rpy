...
init python:
    ...
    dayofweek = 5 #Keeps track of Sun-Sat. 0 = Sunday, 6 = Saturday.
    month = 5 #Current month. 1 = January, 12 = December
    ...
    ###
    #getRelativeMonth(int mv)
    #
    ###
    def getRelativeMonth(mv):
        global dayofmonth
        global direction
        global month
        global months
        ...
        if newVal < 1:
            newVal = months[11][1] + newVal if month == 0 else months[month - 1][1] + newVal
            newMonth = 11 if month == 0 else month - 1
        elif newVal > months[month][1]:
            newMonth = month+1

        return months[newMonth][0]

    def getRelativeTimeOfDay(mv):
      newVal = currentTimeOfDay + mv
      #if newVal >= len(TimeOfDay):
      #  newVal = newVal - (len(TimeOfDay)-1)
        
      #if newVal < 0:
      #  newVal = len(TimeOfDay)- newVal
    ...
    def showCurrentDays(direction, imgSize, posX, posY, displayFullName, startDelay=0):
        ...
        for i in xrange(-2, 6):
            ...
            cMonth = "{size=24}"+getRelativeMonth(i)+"{/size}"
            ...
            if startDelay > 0:
                ...
                ui.add(At(Text(cDay), move_left_align_wait(direction, imgSize, posX-3, posY-dayYOffset, startDelay)))
                ui.add(At(Text(cMonth), move_left_align_wait(direction, imgSize, posX-3, posY-monthYOffset, startDelay)))
                ui.add(At('weather_rain', move_left_align_wait(direction, imgSize, posX, posY-weatherYOffset, startDelay)))
            else:
                ...
                ui.add(At(Text(cDay), move_left_align(direction, imgSize, posX-3, posY-dayYOffset)))
                ui.add(At(Text(cMonth), move_left_align(direction, imgSize, posX-3, posY-monthYOffset)))
                ui.add(At('weather_rain', move_left_align(direction, imgSize, posX, posY-weatherYOffset)))

            posX += imgSize
            
    def moveTime(direction=1):
      global currentTimeOfDay
      newVal = currentTimeOfDay + (1*direction)
      #if newVal >= len(TimeOfDay):
      #  newVal = newVal - (len(TimeOfDay)-1)
    ...
    def showCurrentTime(direction, imgSize, posX, posY, displayFullName, startDelay=0):
        direction = 1 if direction > 0 else -1
        posX -= imgSize
        for i in xrange(-2, 6):
            relDay = "{size=28}"+getRelativeTimeOfDay(i)+"{/size}"
            ...
            if startDelay > 0:
                ...
                #ui.add(At('dayButton', move_up_align(direction, imgSize, posX, posY)))
                ui.add(At('dayButton', move_left_align(direction, imgSize, posX, posY)))
                #ui.add(At(Text(relDay), move_up_align(direction, imgSize, posX, posY)))
                ui.add(At(Text(relDay), move_left_align(direction, imgSize, posX, posY)))
                #ui.add(At(Text(cDay), move_up_align(direction, imgSize, posX-3, posY-98)))
...
transform move_up_align(direction, distance, xPos, yPos):
    xalign (xPos + 80) ycenter (yPos + 80) xcenter (xPos + 80)
    linear 1.0 ycenter (yPos + 80 - direction * distance)
###
#Image resources
###
#image calendar_bg = "calendar/1280x720/bg.png" #Background image
image dayButton = "calendar/1280x720/blue.png" #Image representing each week day
image indicator = "images/ui/Calendar/Calendar Day SelectFull.png" 
image calendar_bg = "images/ui/Calendar/Calendar BG.png" #Background image
#image dayButton = "images/ui/Calendar/Calendar Day Select.png" #Image representing each week day
image weather_rain = "images/ui/Calendar/rain.png" #Image representing each week day
image weather_rain_black = "images/ui/Calendar/Rain Black.png" #Image representing each week day

label calendar(toMove=1):
    ...
    python:
        ...
        imgSize = newsize
        baseX = 50#124 #TODO: move 62px right;  478y 154h
        posY = 600#520
        posX = baseX
        dayYOffset = 598
        monthYOffset = 268
    ...
    while direction != 0:
        ...
        $ showCurrentDays(direction, imgSize, posX, posY, displayFullName, startDelay)
        #$ showCurrentTime(direction, imgSize, posX, posY, displayFullName, startDelay)
    ...
    show weather_rain_black as db_1 at fade_in_immediately(posX, posY-weatherYOffset)
    show text ("{size=28}{color=#000000ff}"+getRelativeDay(-1)+"{/color}{/size}") as dr_1 at fade_in_immediately(posX, posY)
    show text ("{size=24}{color=#000000ff}"+str(months[month][0])+"{/color}{/size}") as dm_1 at fade_in_immediately(posX-3, posY-monthYOffset )
    show text ("{size=24}{color=#000000ff}"+getRelativeWeekDay(-1, displayFullName)+"{/color}{/size}") as dc_1 at fade_in_immediately(posX-3, posY-dayYOffset)
    $ posX += imgSize

    #show dayButton as db_2 at fade_in_immediately(posX, posY)
    show weather_rain as db_2 at fade_in_immediately(posX, posY-weatherYOffset)
    show text ("{size=28}"+getRelativeDay(0)+"{/size}") as dr_2 at fade_in_immediately(posX, posY)
    show text ("{size=24}"+str(months[month][0])+"{/size}") as dm_2 at fade_in_immediately(posX-3, posY-monthYOffset )
    show text ("{size=24}"+getRelativeWeekDay(0, displayFullName)+"{/size}") as dc_2 at fade_in_immediately(posX-3, posY-dayYOffset)
    $ posX += imgSize

    #show dayButton as db_3 at fade_in_immediately(posX, posY)
    show weather_rain as db_3 at fade_in_immediately(posX, posY-weatherYOffset)
    show text ("{size=28}"+getRelativeDay(1)+"{/size}") as dr_3 at fade_in_immediately(posX, posY)
    show text ("{size=24}"+str(months[month][0])+"{/size}") as dm_3 at fade_in_immediately(posX-3, posY-monthYOffset )
    show text ("{size=24}"+getRelativeWeekDay(1, displayFullName)+"{/size}") as dc_3 at fade_in_immediately(posX-3, posY-dayYOffset)
    $ posX += imgSize

    #show dayButton as db_4 at fade_in_immediately(posX, posY)
    show weather_rain as db_4 at fade_in_immediately(posX, posY-weatherYOffset)
    show text ("{size=28}"+getRelativeDay(2)+"{/size}") as dr_4 at fade_in_immediately(posX, posY)
    show text ("{size=24}"+str(months[month][0])+"{/size}") as dm_4 at fade_in_immediately(posX-3, posY-monthYOffset )
    show text ("{size=24}"+getRelativeWeekDay(2, displayFullName)+"{/size}") as dc_4 at fade_in_immediately(posX-3, posY-dayYOffset)
    $ posX += imgSize

    #show dayButton as db_5 at fade_in_immediately(posX, posY)
    show weather_rain as db_5 at fade_in_immediately(posX, posY-weatherYOffset)
    show text ("{size=28}"+getRelativeDay(3)+"{/size}") as dr_5 at fade_in_immediately(posX, posY)
    show text ("{size=24}"+str(months[month][0])+"{/size}") as dm_5 at fade_in_immediately(posX-3, posY-monthYOffset )
    show text ("{size=24}"+getRelativeWeekDay(3, displayFullName)+"{/size}") as dc_5 at fade_in_immediately(posX-3, posY-dayYOffset)
    $ posX += imgSize

    #show dayButton as db_6 at fade_in_immediately(posX, posY)
    show weather_rain as db_6 at fade_in_immediately(posX, posY-weatherYOffset)
    show text ("{size=28}"+getRelativeDay(4)+"{/size}") as dr_6 at fade_in_immediately(posX, posY)
    show text ("{size=24}"+str(months[month][0])+"{/size}") as dm_6 at fade_in_immediately(posX-3, posY-monthYOffset )
    show text ("{size=24}"+getRelativeWeekDay(4, displayFullName)+"{/size}") as dc_6 at fade_in_immediately(posX-3, posY-dayYOffset)
    $ posX += imgSize

    #show dayButton as db_7 at fade_in_immediately(posX, posY)
    show weather_rain as db_7 at fade_in_immediately(posX, posY-weatherYOffset)
    show text ("{size=28}"+getRelativeDay(5)+"{/size}") as dr_7 at fade_in_immediately(posX, posY)
    show text ("{size=24}"+str(months[month][0])+"{/size}") as dm_7 at fade_in_immediately(posX-3, posY-monthYOffset )
    show text ("{size=24}"+getRelativeWeekDay(5, displayFullName)+"{/size}") as dc_7 at fade_in_immediately(posX-3, posY-dayYOffset)
...
label TimeOfDay(toMove=1):
    ...
    python:
        ...
        direction = toMove
        monthPosX = 65 #Y position (in px) of month label to display
        monthPos = 550
        month -= 1 #So users can set month as 1-12 instead of 0-11
        oldmonth = month
        oldyear = year
        ...
        imgSize = newsize
        baseX = 124 #TODO: move 62px right;  478y 154h
        posY = 520
        posX = baseX
    ...
    show dayButton as db_1 at fade_in_immediately(posX, posY)
    show text ("{size=28}"+getRelativeTimeOfDay(-1)+"{/size}") as dr_1 at fade_in_immediately(posX, posY)
    $ posX += imgSize

    show dayButton as db_2 at fade_in_immediately(posX, posY)
    show text ("{size=28}"+getRelativeTimeOfDay(0)+"{/size}") as dr_2 at fade_in_immediately(posX, posY)
    $ posX += imgSize

    show dayButton as db_3 at fade_in_immediately(posX, posY)
    show text ("{size=28}"+getRelativeTimeOfDay(1)+"{/size}") as dr_3 at fade_in_immediately(posX, posY)
    $ posX += imgSize

    show dayButton as db_4 at fade_in_immediately(posX, posY)
    show text ("{size=28}"+getRelativeTimeOfDay(2)+"{/size}") as dr_4 at fade_in_immediately(posX, posY)
    $ posX += imgSize

    show dayButton as db_5 at fade_in_immediately(posX, posY)
    show text ("{size=28}"+getRelativeTimeOfDay(3)+"{/size}") as dr_5 at fade_in_immediately(posX, posY)
    $ posX += imgSize

    show dayButton as db_6 at fade_in_immediately(posX, posY)
    show text ("{size=28}"+getRelativeTimeOfDay(4)+"{/size}") as dr_6 at fade_in_immediately(posX, posY)
    $ posX += imgSize

    show dayButton as db_7 at fade_in_immediately(posX, posY)
    show text ("{size=28}"+getRelativeTimeOfDay(5)+"{/size}") as dr_7 at fade_in_immediately(posX, posY)
    ...
    return# Ren'Py automatically loads all script files ending with .rpy. To use this
