...
init -1 python hide:
    ...
    style.window.background = "img/ui/textbox.png"
    ...
    style.window.left_padding = 00
    style.window.right_padding = 80
    #style.window.top_padding = 60
    style.window.bottom_padding = 60

    ## This is the minimum height of the window, including the margins
    ## and padding.

    style.window.yminimum = 261
    ...
    config.main_menu_music = "sound/bgm/closeto.ogg"
    ...
    config.game_main_transition = Dissolve (1.0, alpha=True)

    ## Used when entering the main menu from the splashscreen.
    config.end_splash_transition = Dissolve (2.0)

    ## Used when entering the main menu after the game has ended.
    config.end_game_transition = Dissolve (1.0, alpha=True)

    ## Used when a game is loaded.
    config.after_load_transition = Dissolve (1.0, alpha=True)
    ...
    config.adv_nvl_transition = Dissolve (1.0, alpha=True)

    ## Used when showing ADV-mode text directly after NVL-mode text.
    config.nvl_adv_transition = Dissolve (1.0, alpha=True)
...
python early:
    config.save_directory = "Dor-1425827805"
...
init python:
    ...
    build.directory_name = "Dor-0.1"

    ## The name that's uses for executables - the program that users will run
    ## to start the game. For example, if this is 'mygame', then on Windows,
    ## users can click 'mygame.exe' to start the game.
    build.executable_name = "Dor"
