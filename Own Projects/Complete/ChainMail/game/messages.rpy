...
init python:
    ...
    reply_screen = False
    forward_screen = False
    draft_screen = False

    class Mail(store.object):
        def __init__(self, subject, sender, body, reply_label=False, forward_label=False, delay=False, view=True, read=False, sent=False, junk=False, recipient=None):
        ...
        def forward(self):
            global reply_screen
            forward_screen = True
            renpy.call_in_new_context(self.forward_label, current_message=self)  
            forward_screen = False     
            
        def updateReplyLabel(self, new_forward_label):
            self.reply_label = new_forward_label
            
        def callForward(self, new_forward_label):
            global reply_screen
            forward_screen = True
            #self.forward_label = new_forward_label
            renpy.call_in_new_context(new_forward_label, current_message=self)  
            forward_screen = False     
    ...
    def add_message(subject, sender, body, reply_label=False, forward_label=False, delay=False):
        renpy.sound.play("audio/chime.wav")
        #renpy.sound.play("audio/377017__elmasmalo1__notification-pop.wav")
        message = Mail(subject, sender, body, reply_label, forward_label, delay)
        #renpy.show_screen("mailbox_overlay")
        
    def add_later(subject, sender, body, reply_label=False, forward_label=False):
        mail_later.append([subject, sender, body, reply_label, forward_label])
              
    def add_now():
        for element in mail_later:
            add_message(element[0], element[1], element[2], element[3], element[4])
        global mail_later
        mail_later = []
        
    def add_next():
        if mail_later:
          element = mail_later.pop()
          add_message(element[0], element[1], element[2], element[3], element[4])
          #global mail_later
          #mail_later = []
...
screen contacts(current_message=None, forwarding=False):
    modal True
    add "gui/overlay/email_menu.png"
    #add "images/ui/email/Compose Text.png"
    #text "this is contacts screen"
    ...
    vbox:
        ...
        text "Who should I send this to?" xpos 50 ypos 150
    #frame:
    #  style_group "pref"
    #  background None
    #  xalign 0.6
    ...
    vbox:
      ...
      for name in contacts:
          if name.draft_label:
            if forwarding:
              textbutton name.name action Return(name.draft_label)
            else:
              textbutton name.name action [name.draft, Hide("contacts")]#, ShowMenu("draft_menu", name.draft)]
          else:
              textbutton name.name action None
      textbutton "Nevermind..." action [Hide("contacts")]
...
#              #ypos 280
#              if reply_screen:
#                  text ("{color=#000}" + GetAddressFromSender("Simone") + "{/color}") xpos 150   
#                  text ("{color=#000}" + GetAddressFromSender(current_message.sender) + "{/color}") xpos 150 ypos 5  
#                  text ("{color=#000}RE: " + current_message.subject + "{/color}") xpos 150 ypos 5            
#                  #text ("")             
#                  #text ("{color=#000}" + current_message.intro + "{/color}")
#              else:
#                  text ("{color=#000}" + GetAddressFromSender("Simone") + "{/color}") xpos 150 
#                  text ("{color=#000}" + GetAddressFromSender(contact.name) + "{/color}") xpos 150 ypos 5
#                  if forward_screen:
#                    text ("{color=#000}FW: " + current_message.subject + "{/color}") xpos 150 ypos 5
#                  else:
#                    text ("{color=#000}" + message_title + "{/color}") xpos 150 ypos 5       
#                  #text ("")                        
