...
screen countdown:
    if TimerActive:
      if (minutes >= timer_end_limit):
        $TimerActive = False
        timer 0.1 action Jump('lbl_end')
      else:
        timer 0.1 repeat True action If(timer_limit > 0, true=SetVariable('timer_limit', timer_limit - 0.1), false=[ Hide('countdown'), Hide("clock"),  SetVariable('minutes', minutes + 1), Show("clock"),  SetVariable('timer_limit', timer_range), Play("audio", sound_tick)]) 
