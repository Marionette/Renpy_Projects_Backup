...
init -1 python hide:
    ...
    config.window_title = u"AzureProject"

    # These control the name and version of the game, that are reported
    # with tracebacks and other debugging logs.
    config.name = "AzureProject"
    config.version = "0.0"

    config.layers = [ 'master', 'b', 'm', 'f', 'texture', 'transient', 'screens', 'overlay' ]
...
python early:
    config.save_directory = "AzureProject-1423847833"
...
init python:
    ...
    build.directory_name = "AzureProject-0.04"

    ## The name that's uses for executables - the program that users will run
    ## to start the game. For example, if this is 'mygame', then on Windows,
    ## users can click 'mygame.exe' to start the game.
    build.executable_name = "AzureProject"
